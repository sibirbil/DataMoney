...
#### Read the file
filename <- "plaintext_whitepapers_corpus_redacted.txt"
...
# To save 
ggsave("term_frequencies_wp.png", dpi = 900)
...
# Plot
ggplot(dftest_part[1:43,], aes(y = reorder(String, Frequency), x = Frequency)) +
  theme_bw() + geom_point(size=3, alpha=0.7, colour="black") + 
  xlab("Frequency") + ylab("") + 
  theme(axis.text.x = element_text(angle = 0, hjust = 1, face = "bold")) +
...
# To save 
ggsave("social_science_term_frequencies_wp.png", dpi = 900)
